...
import com.hmn.indianmtv.model.Pojo
import com.hmn.indianmtv.network.Api
import com.hmn.indianmtv.network.RetrofitHelper
...
class SplachActivity : AppCompatActivity() {
    ...
    private fun networkCall() {
        ...
        if (isConnectingToInternet(this)) {
            if (list.isEmpty()) {
                //RetrofitHelper.getRetrofit<Api>()
                ...
                RetrofitHelper.getRetrofit<Api>().getModel().enqueue(object : Callback<Pojo> {
                    override fun onResponse(call: Call<Pojo>, response: Response<Pojo>) {
                        if (response.isSuccessful) {
                            ...
                            mViewModel.repository.insertAllBanner(catList)
                        }
